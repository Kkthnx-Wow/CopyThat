...
function moduleMeta:RegisterEvent(event, handler)
	...
	assert(type(handler) == "function", ("CopyThat: no handler for event '%s' on module '%s'"):format(event, self.name))
...
function ns:NewModule(name, dbKey, opts)
	assert(not moduleByName[name], ("CopyThat: module '%s' already exists"):format(name))
